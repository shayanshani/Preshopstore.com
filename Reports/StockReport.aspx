<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="StockReport.aspx.cs" Inherits="PreShop.StockManagement.Store.Reports.StockReport" %>
                        ...
                        <asp:Repeater ID="rptStockReport" runat="server">
                            <ItemTemplate>
                                <tr>
                                    ...
                                    <td class="desc"><%# Convert.ToDateTime(Eval("[Purchase_Date]")).ToShortDateString() %></td>
                                    <td class="desc"><%# Eval("[Product]") %>  <%# Eval("[Size]") %></td>
                                    <td class="desc"><%# Eval("[Company]") %></td>
                                    <td class="desc"><%# Eval("[Quantity]") %></td>
                                    <td class="desc"><%# FormatCurrencyDisplay(Eval("[ItemPP]").ToString()) + " x " + Eval("[Quantity]") %></td>
                                    <%--<td class="desc"><%# FormatCurrencyDisplay(Eval("[Expense]").ToString()) %></td>--%>
                                    <td class="desc"><%# FormatCurrencyDisplay(Eval("[Amount]").ToString()) %></td>
                                </tr>
                            </ItemTemplate>
                            <FooterTemplate>
                                <tr runat="server" visible='<%# rptStockReport.Items.Count == 0 %>'>
